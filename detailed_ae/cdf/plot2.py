...
for i in ["unv","enp"]:
    for j in ["dringsu2","dringsu2e","leafspine","dringfhi"]:
        outfile = f"outfiles/{i}_{j}_load3_seed1.out"
        if os.path.exists(f"{outfile}"):
            durationlist = list()
            sizelist = list()
            startlist = list()
            with open(f"{outfile}",'r') as fr:
                lines = fr.readlines()
                for line in lines:
                    tokens = line.split()
                    if tokens[0] == "FCT":
...
# plot
plottitledict = {"unv":"UNV","enp":"ENP","cluster_a":"DB","cluster_b":"WS","cluster_c":"HD"}
...
for icm,cm in enumerate(["unv","enp"]):
    for isc,scheme in enumerate(["dringsu2","dringsu2e","leafspine","dringfhi"]):
        outfile = f"outfiles/{cm}_{scheme}_load3_seed1.out"
        # print(outfile)
        # if scheme == "dringsu2e":
...
plt.savefig(f"fig15.pdf", dpi=600, bbox_inches='tight')