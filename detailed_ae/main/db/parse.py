        ...
        if os.path.exists(f"outfiles/{j}_load{load_list[0]}_seed{seed_list[0]}.out"):
            for load in load_list:
                for seed in seed_list:
                    ...
                    durationlist.sort()
                    # print(outfile)
                    fw.write(f"cluster_a/{outfile}\t{sum(durationlist)/len(durationlist)}\t{durationlist[int(len(durationlist)*0.5)]}\t{durationlist[int(len(durationlist)*0.99)]}\t{durationlist[int(len(durationlist)*0.9999)]}\n")