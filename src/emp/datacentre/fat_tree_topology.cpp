...
FatTreeTopology::FatTreeTopology(Logfile* lg, EventList* ev, queue_type qt){
  logfile = lg;
  eventlist = ev;
  qtype = qt;
  
  N = (K * K * K * OVERSUBSCRIPTION) /4;
...
void FatTreeTopology::init_network(){
    ...
    for (int j = 0; j < NK; j++) {
        for (int l = 0; l < (K/2) * OVERSUBSCRIPTION; l++) {
            int k = j * (K/2) * OVERSUBSCRIPTION + l;
            int delay = 1000;
            // Downlink
    ...
    for (int j = 0; j < NK; j++) {
        ...
        for (int l = 0; l < K/2; l++) {
            ...
            queues_nup_nc[j][k] = alloc_queue(queueLogger, HOST_NIC, queue_size); //new RandomQueue(speedFromPktps(HOST_NIC), memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER), *eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
            ...
            //else
            queues_nc_nup[k][j] = alloc_queue(queueLogger, HOST_NIC, queue_size); //new RandomQueue(speedFromPktps(HOST_NIC), memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER), *eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
...
int FatTreeTopology::get_distance(int src, int dest){
  if (HOST_POD_SWITCH(src)==HOST_POD_SWITCH(dest)) return 0;
  else if (HOST_POD(src)==HOST_POD(dest)) return 2;
  else return 4;
}



pair<vector<double>*, vector<route_t*>*> FatTreeTopology::get_other_paths(int src, int dest){
	return get_paths(src, dest);
}

pair<vector<double>*, vector<route_t*>*> FatTreeTopology::get_paths(int src, int dest){
  ...
  if (HOST_POD_SWITCH(src)==HOST_POD_SWITCH(dest)){
    ...
    for (int upper = MIN_POD_ID(pod);upper <= MAX_POD_ID(pod); upper++){
      int npods = MAX_POD_ID(pod) - MIN_POD_ID(pod) + 1, minpod = MIN_POD_ID(pod);
      upper = minpod + ((upper-minpod) + (rand()%npods))%npods;
      for (int core = (upper%(K/2)) * K / 2; core < ((upper % (K/2)) + 1)*K/2; core++){
	int ncores = ((upper % (K/2)) + 1)*K/2 - (upper%(K/2)) * K / 2, mincore = (upper%(K/2)) * K / 2;
	core = mincore + ((core-mincore) + (rand()%ncores))%ncores;
	//upper is nup
	Queue* pqueue = new Queue(speedFromPktps(HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, NULL);
	pqueue->setName("PQueue_" + ntoa(src) + "_" + ntoa(dest));
	//logfile->writeName(*pqueue);
	...
	//printf("core: %d, HPS(src): %d, HPS(dest): %d, upper: %d, upper2: %d\n", core, HOST_POD_SWITCH(src), HOST_POD_SWITCH(dest), upper, upper2);
	if((HOST_POD_SWITCH(src) + HOST_POD_SWITCH(dest) + core)%(K/2) == 0 || !USE_DISJOINT){
		paths->push_back(routeout);
		check_non_null(routeout);
	}
	break;
...
void FatTreeTopology::print_path(std::ostream &paths,int src,route_t* route){
  paths << "SRC_" << src << " ";
